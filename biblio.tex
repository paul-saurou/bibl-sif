...
\abstract{
    ...
    This type of attack is called side-channel analysis (SCA).
    ...
    However, SCA can also be used to reverse engineer some parts or the whole structure of a cryptographic algorithm.
    This is called Side-Channel Analysis for Reverse-Engineering (SCARE).
    The purpose of this internship is to evaluate the potential threat of using SCARE techniques on a hardware implementation of an SPN encryption scheme.
    In this bibliographic review, we introduce the context of the internship, as well as prospective issues and state-of-the-art SCARE techniques on both software and hardware cryptographic systems.
}
...
SCA constitute a powerful tool to reverse-engineer some secret parts of a cryptographic implementation.
This is called Side-Channel Analysis for Reverse-Engineering (SCARE) and it aims at recovering information about a cryptographic process (and not the manipulated data as usually targeted in side-channel attacks) based on the device power consumption.
...
Another very popular category of symmetric cryptography algorithms is substitution–permutation networks (SPN).
The most used SPN is the Advanced Encryption Standard (AES). %TODO citation ?
The SCARE possibilities for hardware SPNs have almost not been studied yet, thus this internship aims at completing the SCARE studies for SPNs.

More specifically, the purpose of this internship is to evaluate the potential threat of using SCARE techniques on a hardware implementation of an SPN encryption scheme.
To do that, a good understanding of existing side channel attacks on both software and hardware implementations is needed.
These attacks can be against SPNs specifically or other similar symmetric cryptographic algorithms.
The goal is to propose new attack paths for the SPN hardware case and assess their relevance based on standardized benchmarks.
...
In this bibliographic review, we first define the ciphers and the main SCA techniques that will be used later.
Secondly, we present an overview of SCARE methods and their applications.
The analyses are separated depending on the cipher they target and their attacker model.
Thirdly, we focus on hardware implementations and explain the differences between software and hardware SCA and why there are fewer studies on hardware implementations.
We present the published hardware SCARE attacks and show the lack of literature for SPN hardware SCARE.
...
Then, we define several Side Channel Analysis state-of-the-art attacks on these implementations.
Finally, we present the most common countermeasures to SCA. 
...
They must not be mixed up with substitution-permutation networks that are a completely different kind of cipher.
...
\begin{center}
    ...
    \captionsetup{hypcap=false}
    \captionof{figure}{A sketch of a substitution–permutation network with 3 rounds, encrypting a plaintext block of 16 bits into a ciphertext block of 16 bits. The S-boxes are the $S_i$, and the round keys are the $K_i$.}
    \label{fig_spn}
\end{center}
...
It is an SPN that exists in three variants:
\begin{itemize}
...
\begin{center}
    ...
    \captionsetup{hypcap=false}
    \captionof{figure}{A sketch of a round of AES. S is the S-box and $K_i$ the round key.}
    \label{fig_aes}
\end{center}
...
Deep Neural Networks (DNNs) are able to efficiently break implementations of AES that resisted to other types of SCA, particularly when desynchronization occurs \parencite{Maghrebi_Portigliatti_Prouff_2016}.
...
\subsection{SCARE on hardware design}
...
In 2010, Guilley et al. presented a SCARE of a DES implemented on FPGA \parencite{Guilley_Sauvage_Micolod_Réal_Valette_2010}.
They used CPA and the attack has already been detailed in section \ref{scare_des}.
...
This bibliographic report provides a comprehensive overview of block ciphers, SCA techniques, and SCARE methods and highlights the need for further research in this area.

The studies reviewed propose a synthetic overview of SCA in terms of methods, leakages models, assumptions made and countermeasures.
The applications of SCA to reverse engineering are more recent and cover a wide range of possible scenarios, from recovering part of the parameters in a known architecture to discovering everything about a completely unknown cipher.
When the cryptographic algorithm under study is implemented on hardware, SCA and SCARE are harder to do and to replicate.
Efforts to standardize hardware SCA benchmarks have been made, notably during the DPA contest \parencite{Clavier_Danger_Duc_Elaabid_Gérard_Guilley_Heuser_Kasper_Li_Lomné_et_al_2014}.
Today, many studies on FPGAs use the same SASEBO capture board \parencite{Satoh}.

The current literature focuses mainly on software implementations and Feistel ciphers, with very little attention paid to hardware SPN implementations.
This internship aims to address this gap by evaluating the potential threat of SCARE on a hardware implementation of an SPN encryption scheme.
To achieve this goal, theoretical attack paths will be proposed and tested on a simulated cipher. 
Then a more practical part of the internship will be to use a capture board to get power consumption leakages and clean the hardware platform signals to adapt the proposed attacks to real traces.
