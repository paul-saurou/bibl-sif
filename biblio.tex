...
An encryption algorithm takes plaintext and key as input and outputs a ciphertext, that can be deciphered with another secret key to retrieve the plaintext.
For a symmetric algorithm, the encryption and the decryption keys are the same. 
The confidentiality property states that the ciphertext must be indistinguishable from a random stream of bytes for someone that does not know the decryption key.
...
\begin{center}
    ...
    \captionsetup{hypcap=false}
    \captionof{figure}{A sketch of a substitution–permutation network with 3 rounds, encrypting a plaintext block of 16 bits into a ciphertext block of 16 bits. The S-boxes are the Si, and the round keys are the Ki. Image inspired by wikipedia.org}
    \label{fig_spn}
\end{center}
...
It can also be adapted to circumvent simple masking.
