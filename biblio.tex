...
\abstract{
% Today, the security of modern cryptographic algorithms relies on difficult mathematical problems or can be proven mathematically.
% However, these proofs apply only to "black-box" adversaries, even if they are implemented in software or hardware modules.
% These modules are physically accessible, and attackers can retrieve secret information by observing and exploiting their physical leakages (e.g. power consumption).
...
% Furthermore, SCA constitute a powerful tool to reverse-engineer some secret parts of a cryptographic implementation.
...
% While SCARE attacks mainly target software implementations, the few works on hardware implementations are focused on Feistel constructions and only a few elements are provided for Substitution Permutation Networks.
...
A cryptographic algorithm takes plaintext and key as input and outputs a ciphertext, that can be deciphered with another secret key to retrieve the plaintext. % peut être retirer to retrieve the plaintext
For a symmetric algorithm, the encryption and the decryption keys are the same. % If the encryption and decryption keys are the same, we say that the algorithm is symmetric / Some of these algo are said to be symmetric, which means that ...
The confidentiality property states that the ciphertext must be indistinguishable from a random stream of bytes for someone that does not know the deciphering key.
